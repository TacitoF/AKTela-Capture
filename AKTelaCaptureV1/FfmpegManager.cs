    ...
    public static async Task<string> EnsureAsync(IProgress<int>? progress = null, CancellationToken token = default)
    {
        ...
        try
        {
            ...
            try
            {
                ...
                http.DefaultRequestHeaders.UserAgent.Add(new ProductInfoHeaderValue("AKTelaCapture", "2.9"));
                ...
                if (!await SupportsGfxCaptureAsync(found, token))
                    throw new InvalidOperationException("O FFmpeg baixado não oferece a captura moderna e o redimensionamento pela GPU necessários.");
    ...
    internal static async Task<bool> SupportsGfxCaptureAsync(string executable, CancellationToken token = default)
    {
        ...
        try
        {
            ...
            return process.ExitCode == 0 &&
                   output.Contains("gfxcapture", StringComparison.Ordinal) &&
                   output.Contains("scale_d3d11", StringComparison.Ordinal);
        }
        catch
        {
